...
FoundScriptType:
		...
		Catch
			MessageBox.Show(Me, "Something went wrong! The current AI Box may not be formatted correctly or a path may be inaccessible!" & Environment.NewLine & Environment.NewLine &
	...
	Private Sub Button2_Click(sender As System.Object, e As System.EventArgs) Handles BTNSaveFile.Click
		...
		If File.Exists(aiFile) Then
			...
			If aiScriptType <> "Response" And aiScriptType <> "List" Then
				...
				Dim Result As Integer = MessageBox.Show(Me, Path.GetFileName(aiFile) & " already exists! Do you wish to overwrite?" & Environment.NewLine & Environment.NewLine &
